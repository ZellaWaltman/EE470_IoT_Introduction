  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (https.begin(client, fullUrl)) { // start the connection 1=started / 0=failed
      ...
      if (httpCode > 0) {
        ...
        if (!err) {
          mytime = doc["dateTime"].as<String>(); // "2025-10-18T22:31:44.1790514"
            int dot = mytime.indexOf('.'); // find the decimal point
            if (dot > 0) mytime = mytime.substring(0, dot); // keep up to seconds → "2025-10-18T22:31:44"
            Serial.println("ISO trimmed: " + mytime);
